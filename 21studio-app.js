...
(() => {
  ...
  var BrainCluster = class {
    ...
    _paint(t) {
      ...
      glow.addColorStop(0, "rgba(214,85,63,0.22)");
  ...
  var RadialGraph = class {
    ...
    _layout() {
      const size = Math.min(this.root.clientWidth, 640);
      ...
      this.departments.forEach((dep, i) => {
        ...
        const x = this.cx + Math.cos(angle) * this.opts.radius;
        const y = this.cy + Math.sin(angle) * this.opts.radius * 0.86;
    ...
    _drawLines() {
      ...
      this.departments.forEach((dep) => {
        ...
        line.setAttribute("opacity", this.activeId ? isActive ? "0.9" : "0.12" : "0.28");
